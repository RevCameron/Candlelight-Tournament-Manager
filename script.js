...
function confirmStartTournament() {
  ...
  if (confirm("Start the tournament? This will lock the player list and generate Round 1.")) {
    startRounds();
  }
}
...
function printFinalStandings() {
  ...
  const isFinal = tournament.currentRound === tournament.totalRounds && isRoundComplete(tournament.currentRound);
  const headerText = isFinal 
    ? "Final Standings" 
    : `Standings ${tournament.name} Round ${tournament.currentRound}/${tournament.totalRounds}`;

  const html = `
    <html><head><title>${headerText}</title>
  ...
  openPrintWindow(headerText, html);
}
